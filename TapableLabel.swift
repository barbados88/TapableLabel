...
class TapableLabel: UILabel, UIGestureRecognizerDelegate {
    ...
    var pFont: UIFont = UIFont.systemFont(ofSize: 14, weight: .medium)
    var aFont: UIFont = UIFont.systemFont(ofSize: 14, weight: .bold)
    ...
    @IBInspectable var partText: String = NSLocalizedString(": additional text here", comment: "") {
        didSet {
            lText = partText
        }
    }
    ...
    @objc private func singleTap(_ gesture: UITapGestureRecognizer) {
        ...
        case .changed, .ended, .cancelled, .failed:
            indicateTap()
            if tapBlock != nil { tapBlock?(pText) }
        }
    }
